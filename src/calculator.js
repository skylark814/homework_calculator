            ...
            let mouseListener = {
                ...
                onMouseDown: function (event) {
                    ...
                    if (cc.rectContainsPoint(layer.plusRect, point)) {
                        ...
                        if (layer.formula !== "") {
                            if (layer.formula.charAt(layer.formula.length - 1) !== "+" &&
                                layer.formula.charAt(layer.formula.length - 1) !== "-" &&
                                layer.formula.charAt(layer.formula.length - 1) !== "*" &&
                                layer.formula.charAt(layer.formula.length - 1) !== "/") {
                                layer.formula += "+";
                            }
                    ...
                    if (cc.rectContainsPoint(layer.equalRect, point)) {
                        ...
                        if (layer.formula !== "") {
                            let mod = layer.formula.split("/");
                            if (mod.length === 2 && layer.formula.indexOf("+") === -1
                                && layer.formula.indexOf("-") === -1 && layer.formula.indexOf("*") === -1) {
                                layer.formula = parseInt(mod[0] / mod[1]) + "..." + mod[0] % mod[1];
                            }
                            else {
                        ...
                        if(layer.formula!==""){
                            for (let i = 0; i < layer.rects.length; i++) {
                                if (cc.rectContainsPoint(layer.rects[i], point)) {
                                    ...
                                    layer.formula += i;
                                    if(layer.formula.substr(0,1)===0){
                                        layer.formula = layer.formula.slice(1,layer.formula.length);
                                    }
                                    layer.input.setString(layer.formula);
                                    break;
                                }
                            ...
                            for (let i = 1; i < layer.rects.length; i++) {
                                if (cc.rectContainsPoint(layer.rects[i], point)) {
...
let CalScene = cc.Scene.extend({
    onEnter: function () {
        this._super();
        let layer = new CalLayer();
        this.addChild(layer);
    }
});
